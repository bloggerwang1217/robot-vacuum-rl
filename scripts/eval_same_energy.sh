#!/bin/bash

# 機器人吸塵器 RL 批次評估腳本
# 評估虐殺模式：高血量機器人 vs 低血量機器人 的訓練結果

set -e

SCRIPT_DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" && cd .. && pwd )"

# 模型目錄 (使用已訓練好的模型)
MODELS_DIR="${SCRIPT_DIR}/models/epsilon_decay_20251118_112056"

# 選擇要評估的 episode checkpoint (推薦 2000 = 完全收斂)
EPISODE="2000"

# 最大步數 (每個 evaluation 的長度)
MAX_STEPS=10000

echo "=========================================="
echo "機器人吸塵器強化學習 - 虐殺模式評估"
echo "=========================================="
echo "開始時間: $(date)"
echo "模型來源目錄: $MODELS_DIR"
echo "評估 Episode: $EPISODE"
echo "最大步數: $MAX_STEPS"
echo "=========================================="
echo ""

# 定義評估配置 — 使用剛剛從 git 歷史還原的 epsilon_decay 三組同血量 checkpoints
# 格式: "模型子目錄名稱|評估參數 (energy 要與訓練時一致)"
declare -a configs=(
  "collision-10-energy-150-epsilon-decay|--robot-0-energy 150 --robot-1-energy 150 --robot-2-energy 150 --robot-3-energy 150 --e-collision 10"
  "collision-10-energy-200-epsilon-decay|--robot-0-energy 200 --robot-1-energy 200 --robot-2-energy 200 --robot-3-energy 200 --e-collision 10"
  "collision-50-energy-100-epsilon-decay|--robot-0-energy 100 --robot-1-energy 100 --robot-2-energy 100 --robot-3-energy 100 --e-collision 50"
)

total_configs=${#configs[@]}
echo "共有 $total_configs 個評估配置 - 虐殺模式"
echo ""
echo "血量配置 (虐殺模式)："
echo "  1. [1v3] Robot 0: 1000 | Robots 1,2,3: 20 (一個強者 vs 三個弱者)"
echo "  2. [2v2] Robots 0,1: 1000 | Robots 2,3: 20 (兩強 vs 兩弱)"
echo "  3. [3v1] Robots 0,1,2: 1000 | Robot 3: 20 (三強圍殺一個弱者)"
echo ""
echo "終止條件: 存活數 ≤ 1 或達到最大步數"
echo ""

# 遍歷每個配置並執行評估
for ((i=0; i<${#configs[@]}; i++)); do
  config="${configs[$i]}"
  config_name="${config%|*}"
  config_params="${config#*|}"

  # 計算進度
  current=$((i+1))

  # 模型路徑
  model_dir="$MODELS_DIR/$config_name/episode_$EPISODE"

  # 評估 log 儲存路徑 (與模型放在同一目錄)
  eval_log="$MODELS_DIR/$config_name/eval_ep${EPISODE}.log"

  echo "=========================================="
  echo "[$current/$total_configs] 評估: $config_name"
  echo "模型路徑: $model_dir"
  echo "參數: $config_params"
  echo "Log 儲存: $eval_log"
  echo "開始時間: $(date)"
  echo "=========================================="

  # 檢查模型是否存在
  if [ ! -d "$model_dir" ]; then
    echo "⚠️  警告: 模型目錄不存在: $model_dir"
    echo "跳過此配置..."
    echo ""
    continue
  fi

  # 執行評估並儲存 log (使用 deterministic policy, epsilon=0)
  python3 "$SCRIPT_DIR/evaluate_models.py" \
    --model-dir "$model_dir" \
    $config_params \
    --max-steps $MAX_STEPS \
    --seed 42 \
    --wandb-entity lazyhao-national-taiwan-university \
    --wandb-project robot-vacuum-eval \
    --wandb-run-name "eval-$config_name-ep$EPISODE" \
    --wandb-mode online \
    2>&1 | tee "$eval_log"

  echo ""
  echo "✓ 評估完成: $config_name"
  echo "Log 已儲存到: $eval_log"
  echo "結束時間: $(date)"
  echo ""

  # 評估間隔
  if [ $current -lt $total_configs ]; then
    echo "等待 3 秒後開始下一個評估..."
    sleep 3
  fi
done

echo "=========================================="
echo "所有虐殺模式評估完成！"
echo "結束時間: $(date)"
echo "=========================================="
echo ""
echo "查看結果: https://wandb.ai/lazyhao-national-taiwan-university/robot-vacuum-eval"
